...
def generate_ctl(
    ...
    lines = [
        ...
        f"-- Generated by csv_schema_detector.py",
        "",
        f"OPTIONS (SKIP=1, ERRORS=100, DIRECT=FALSE)",
        f"LOAD DATA",
        f"INFILE '{csv_file}'",
        f"APPEND",
    ...
    if enclosure:
        lines.append(f"         OPTIONALLY ENCLOSED BY '{enclosure}'")
    lines.append("TRAILING NULLCOLS")
    lines.append("(")

    col_lines = []
    for col in schema:
        name_padded    = col.name.upper().ljust(30)
        oracle_type    = col.oracle_type
        max_len        = _varchar_len(oracle_type)   # raw char buffer size

        if oracle_type == "CLOB":
            # CLOB: load as large CHAR buffer, Oracle handles the rest
            col_lines.append(f"  {name_padded}CHAR(65535)")

        elif oracle_type in ("DATE", "TIMESTAMP"):
            mask = col.sqlldr_mask or ("YYYY-MM-DD" if oracle_type == "DATE" else "YYYY-MM-DD HH24:MI:SS")
            if oracle_type == "TIMESTAMP":
                # SQLLDR cannot cast TIMESTAMP natively — load as CHAR then convert
                col_lines.append(
                    f"  {name_padded}CHAR({max_len}) \"TO_TIMESTAMP(:{ col.name.upper() }, '{mask}')\""
                )
            else:
                col_lines.append(
                    f"  {name_padded}CHAR({max_len}) \"TO_DATE(:{ col.name.upper() }, '{mask}')\""
                )

        elif oracle_type.startswith("NUMBER"):
            # Extract precision/scale from NUMBER(p,s) or NUMBER(p)
            m = re.match(r"NUMBER\((\d+)(?:,(\d+))?\)", oracle_type)
            precision = int(m.group(1)) if m else 18
            scale     = int(m.group(2)) if (m and m.group(2)) else 0
            # buffer: sign + digits + decimal separator + scale digits + margin
            buf = precision + scale + 3
            if scale > 0:
                col_lines.append(
                    f"  {name_padded}CHAR({buf}) \"TO_NUMBER(:{ col.name.upper() }, "
            ...
            # VARCHAR2, CHAR — plain CHAR buffer sized to match DDL length
            col_lines.append(f"  {name_padded}CHAR({max_len})")
